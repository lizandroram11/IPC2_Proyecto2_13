...
public class SolicitudService {
    ...
    /*public String procesarSolicitud() {
        ...
        Paquete paquete = paqueteRepository.buscarPorId(solicitud.getPaqueteId());
        if (paquete == null || paquete.getEstado() != EstadoPaquete.PENDIENTE) {
            return "Solicitud fallida: paquete inválido o no pendiente";
        }

        Mensajero mensajeroDisponible = mensajeroRepository.listarTodos().values().stream()
                .filter(Mensajero::estaDisponible)
                .findFirst()
                .orElse(null);

        if (mensajeroDisponible == null) {
            return "Solicitud fallida: no hay mensajeros disponibles";
        }
        ...
        if (ruta == null) {
            return "Solicitud fallida: no existe ruta entre origen y destino";
        }
        ...
        return "Solicitud procesada con éxito: paquete " + paquete.getId() +
                " asignado a mensajero " + mensajeroDisponible.getNombre();
    }*/
